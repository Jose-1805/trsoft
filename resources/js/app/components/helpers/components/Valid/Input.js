...
import { Segment, Form, Grid, TextArea, Popup } from 'semantic-ui-react';
...
class Input extends React.Component {
	 ...
	 */
	validationInput(){
		const vLength = validateLength(this.inputEvent.target.value, this.props.min_length, this.props.max_length, this.props.t);
		...
		if(
			('min' in this.props && this.props.min > parseFloat(this.inputEvent.target.value))
			|| ('max' in this.props && this.props.max < parseFloat(this.inputEvent.target.value))
		){
			this.stopEvent = true;
		}
	...
	handleInputChange(e){
		...
		if(this.stopEvent){
			this.inputEvent.target.value = this.state.last_value;
			...
			this.setState({
				last_value:this.inputEvent.target.value
			});
		}
	...
	render(){
		...
		const {id, name, type, value, label, required, placeholder, max, min} = this.props;
		const propsInput = {id, name, type, value, label, required, placeholder, max, min};

		let errors = "";
		if(!("noRenderFails" in this.props)){
			errors = _.map(this.state.errors, (el, i) => {
						if(typeof el === "string")
		                	return <p key={i} style={{color:this.props.inverted?"#fff":"#9f3a38", marginBottom:"0px"}}>{el}</p>
	                });

			//une los mensajes enviados desde el componente que instancia
			_.map(this.state.otherErrors, (el, i) => {
				if(typeof el === "string")
                	errors.push(<p key={i} style={{color:this.props.inverted?"#fff":"#9f3a38", marginBottom:"0px"}}>{el}</p>)
            });
		}
        ...
        if('help' in this.props){
        	input_render = <Popup content={this.props.help}
					...
					size="mini"
					position="top left"
					inverted={this.props.inverted?true:false}
