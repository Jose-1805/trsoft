...
/**
	 ...
	 * @param  {Function} t   Funcion para traduccioón
	 * @return {Boolean}      Indica si el evento de la entrada debe continuar (mostrar la actualización)
	 */
const validateLength = (value, min = null, max = null, t = null) => {
	...
	if(min && value.length < min){
		...
		validationMsj = value.length?(t?t('valid.min_length', {count:min}):"Este campo debe contener "+min+" caracteres como mínimo."):"";
	}
	...
	if(max && value.length > max){
		...
		stopEvent = true;
		validationMsj = t?t('valid.max_length', {count:max}):"Este campo puede contener "+max+" caracteres como máximo.";
	}
...
const validateRegExp = (value, nameRegExp, updateStateWith = null, context) => {
    ...
    if(regex.test(value)){
    	...
    	setError(updateStateWith?updateStateWith:nameRegExp, (context.props && context.props.t)?context.props.t('valid.'+(updateStateWith?updateStateWith:nameRegExp)):messagesRegExps[updateStateWith?updateStateWith:nameRegExp], context);
